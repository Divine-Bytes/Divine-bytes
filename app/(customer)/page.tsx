    ...
    <div className="flex flex-col">
      ...
      <section className="relative min-h-[85dvh] flex items-center overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-20 text-center w-full">
          <p className="font-body text-luxury-gold text-sm uppercase tracking-widest mb-4">Premium Handcrafted Chocolates</p>
          <h1 className="font-heading text-4xl md:text-6xl text-white mb-6 max-w-2xl mx-auto leading-tight">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/shop" className="inline-flex items-center justify-center h-14 px-8 text-lg rounded-full bg-luxury-gold text-deep-navy font-body font-semibold hover:bg-luxury-gold/90 transition-all duration-200 shadow-lg">Shop Collection</Link>
            <Link href="/about" className="inline-flex items-center justify-center h-14 px-8 text-lg rounded-full border-2 border-white text-white font-body font-medium hover:bg-white hover:text-deep-navy transition-all duration-200">Explore Our Story</Link>
          </div>
        </div>
      </section>
      ...
      {/* Why Divine Bytes */}
      <section className="py-16 bg-deep-navy/5 px-4">
        <div className="max-w-5xl mx-auto">
          <SectionHeader title="Why Divine Bytes" centered className="mb-10" />
            ...
            {[
              {
                ...
                desc: 'We source only the finest chocolate and fresh ingredients for every creation.'
              },
              {
                icon: (
                  <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
                ...
                desc: 'Every piece is made by hand in small batches, finished with precision and love.'
              },
              {
                icon: (
                  <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
                ...
                desc: 'Elegant packaging designed to make every gift feel truly extraordinary.'
              },
            ].map(({ icon, title, desc }) => (
              <div key={title} className="flex flex-col items-center text-center p-6 rounded-2xl bg-white shadow-sm">
                <div className="mb-4 text-luxury-gold">{icon}</div>
                <h3 className="font-heading text-lg text-deep-navy mb-2">{title}</h3>
                <p className="font-body text-gray-500 text-sm">{desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {signatureBar && (
        <section className="py-16 md:py-24 px-4 max-w-5xl mx-auto w-full">
          <div className="flex flex-col md:flex-row items-center gap-10 rounded-3xl bg-chocolate-brown/5 p-8 md:p-12">
            <div className="flex-1">
              <p className="text-luxury-gold text-sm uppercase tracking-widest font-body mb-3">Signature Collection</p>
              <h2 className="font-heading text-3xl text-deep-navy mb-4">The Signature Chocolate Bar</h2>
              <p className="font-body text-gray-500 mb-6">Personalize every detail — choose your chocolate base, add a name, and describe your vision. Crafted exclusively for you.</p>
              <Link href="/product/signature-chocolate-bar" className="inline-flex items-center justify-center h-14 px-8 text-lg rounded-full bg-deep-navy text-white font-body font-medium hover:bg-opacity-90 transition-all duration-200">Customize Yours</Link>
            </div>
            <div className="w-full md:w-72 h-64 rounded-2xl overflow-hidden shrink-0">
